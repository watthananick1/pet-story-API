...
app.post('/users', async (req, res) => {
  ...
  app.post('/images', async (req, res) => {
    try {
      ...
      stream.on('error', (err) => {
        console.error(err);
        res.status(500).send(`An error occurred while uploading the file: ${err.message}`);
      });
      ...
      console.error(error);
      res.status(500).send(`An error occurred while uploading the file: ${error.message}`);
    }
  });
