...
const member = require('./Controller/api/users')

// Define your routes here
app.get("/", (req, res) => {
  res.send("Hello! welcome to API");
});
app.get("api/users", member.userAll)
...
//     res
//       .status(500)
//       .send(`An error occurred while retrieving users: ${error.message}`);
//   }
// });
...
app.delete("/users/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send(`An error occurred while deleting the user: ${error.message}`);
  }
});
...
app.put("/users/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send(`An error occurred while updating the user: ${error.message}`);
  }
});
